function getMinJump(obstacles) {
  ...
  const highestObstacle = sortedObstacles[sortedObstacles.length - 1];

  let jump = highestObstacle + 1; // default jump

  for (let jumpSize = 1; jumpSize <= highestObstacle; jumpSize++) {
    for (let i = 0; i <= highestObstacle; i = i + jumpSize) {
      if (sortedObstacles.includes(i)) {
      break;
      }

      const nextPosition = i + jumpSize;
      if (nextPosition > highestObstacle && jumpSize < jump) {
        jump = jumpSize;
      }
    }
  }
...
nos ha dado la lista de posiciones de los obstáculos desordenada... 
...
Necesitamos una función que nos diga la longitud mínima del salto del trineo para ir
 evitando todos los obstáculos en la ruta. 
 (min multiple of all missing numbers, starting from (min.no. - 1))
