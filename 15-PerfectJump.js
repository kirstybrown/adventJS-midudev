function checkSledJump(heights) {
    ...
    // sleigh never takes off
    if (maxHeight === 0) {
      return false;
    }

    // sleigh goes up but never comes down
    if (maxHeightIndex === (heights.length - 1)) {
      return false;
    }

    // sleigh goes up again after highest jump
    for (let i = maxHeightIndex; i >= 0; i--) {
      if (heights[i - 1] >= heights[i]) {
        return false;
      }
    }

    // sleigh down before reaching highest point
    for (let i = maxHeightIndex + 1; i < heights.length; i++) {
